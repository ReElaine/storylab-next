...
#### 3. Scene 正在从文本控制单元走向状态变换单元

我们已经能控制 scene 怎么写、怎么改；
并且已经接入了初版：

- scene output state fold
- scene 对 chapter delta 的贡献
- 改后 scene 的增量重结算
...
- scene input state
- scene causality hook
- 更细的 power / resource / world-state fold
...
3. scene 级增量重结算和初版 scene output-state fold 已经接入，但还没有完成更完整的 scene-state 模型
...
- `plan-next` 已开始读取 recent theme progression，并把“主题压力推进”转成下一章的主题意图
...
- scene output-state fold 已接入初版：chapter 级 chronology / open loops / reveals / relationships / theme progression 会优先折叠 scene 输出状态
